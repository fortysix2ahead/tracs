...
class Activity( BaseDocument ):
	...
	uids: List[str] = field( init=True, default=[], metadata={ PERSIST: False, PROTECTED: True } ) # uids of activities which belong to this activity

	raw: Any = field( init=True, default=None, metadata={ PERSIST: False, PROTECTED: True } )  # structured raw data used for initialization from external data
	raw_id: int = field( init=True, default=None, metadata= { PROTECTED: True } )  # raw id as raw data might not contain all data necessary
	...
	metadata: Dict = field( init=False, default=None, metadata={ PROTECTED: True } )
	resources: List[Resource] = field( init=False, default=[], metadata={ PROTECTED: True } )
